...
map.on('click', function (e) {
  ...
  // SELECT * FROM nypd_motor_vehicle_collisions WHERE ST_Within(ST_Transform(the_geom, 2263), ST_Buffer(ST_Transform(CDB_LatLng(40.732,-73.986), 2263),10000))--SELECT * FROM WHERE ST_Within(ST_Transform(the_geom, 3857), ST_Buffer(ST_Transform(CDB_LatLng(41.418015,-75.162964), 3857),100000))
  
  // So place the lat and lng in the query at the appropriate points
  var sql = 'SELECT * FROM allprograms321_latlong WHERE ST_Within(ST_Transform(the_geom, 3857), ST_Buffer(ST_Transform(CDB_LatLng(' + e.latlng.lat + ',' + e.latlng.lng + '), 3857),100000))';
  console.log(sql);
  
  producerSource.setQuery(sql);
  
  // Make SQL to get the summary data you want
  var countSql = 'SELECT SUM(total_acreage) FROM allprograms321_latlong WHERE ST_Within(ST_Transform(the_geom, 3857), ST_Buffer(ST_Transform(CDB_LatLng(' + e.latlng.lat + ',' + e.latlng.lng + '), 3857),100000))';
  ...
  reqwest('https://cdepman.carto.com/api/v2/sql/?q=' + countSql, function (response) {
    // All of the data returned is in the response variable
    console.log(response);
    
    // The sum is in the first row's sum variable
    var sumAcres = response.rows[0].sum;
    ...
    sidebarContainer.innerHTML = '<div>There are ' + sumAcres + ' acres of farmland in this area</div>';
  });
});
/*
